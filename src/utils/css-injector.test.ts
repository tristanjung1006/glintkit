...
describe("css-injector.ts", () => {
  ...
  it("should append preset to end of CSS file with markers", () => {
    ...
    expect(result).toContain("/* glintkit:glass - start */");
    expect(result).toContain(presetContent);
    expect(result).toContain("/* glintkit:glass - end */");
  });
  ...
  it("should use correct marker format", () => {
    ...
    expect(result).toContain("/* glintkit:test-preset - start */");
    expect(result).toContain("/* glintkit:test-preset - end */");
  });
...
@tailwind utilities;`;
    ...
    const startMarkerCount = (result.match(/\/\* glintkit:glass - start \*\//g) || []).length;
    const endMarkerCount = (result.match(/\/\* glintkit:glass - end \*\//g) || []).length;
  ...
  it("should allow multiple presets to coexist in same file", () => {
    ...
    expect(result).toContain("/* glintkit:glass - start */");
    expect(result).toContain("/* glintkit:glass - end */");
    expect(result).toContain("/* glintkit:animations - start */");
    expect(result).toContain("/* glintkit:animations - end */");
    expect(result).toContain("/* glintkit:glow - start */");
    expect(result).toContain("/* glintkit:glow - end */");
...
.another-class {
  ...
  it("should handle updating preset multiple times", () => {
    ...
    const startMarkerCount = (result.match(/\/\* glintkit:test - start \*\//g) || []).length;
    const endMarkerCount = (result.match(/\/\* glintkit:test - end \*\//g) || []).length;
    expect(startMarkerCount).toBe(1);
    expect(endMarkerCount).toBe(1);
  });
});
